...
function Sidebar({ opensidebar, setopensidebar }) {
  const menu = [
    ...
    { icon: <Package size={20} />, name: "Products" },
    { icon: <ShoppingCart size={20} />, name: "Orders" },
    { icon: <Users size={20} />, name: "Customers" },
    { icon: <PaintBucket size={20} />, name: "Paint Shades" },
    { icon: <BarChart3 size={20} />, name: "Analytics" },
  ...
  return (
    <aside className={`${opensidebar ? "translate-x-0" : "-translate-x-full"} text-white bg-[#050c19] lg:translate-x-0 fixed left-0 top-0 w-72 h-screen transition-all duration-300 z-50`}>
      ...
      <div className="flex-1 p-4 space-y-3">
        {menu.map((item, i) =>
          <button key={i} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${i === 0 ? "bg-[#13233f]" : "hover:bg-[#13233f]"}`}>
